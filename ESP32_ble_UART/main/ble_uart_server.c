...
extern QueueHandle_t queue_gps_status;


uint8_t char1_str[GATTS_CHAR_VAL_LEN_MAX] = {0x11,0x22,0x33};
uint8_t descr1_str[GATTS_CHAR_VAL_LEN_MAX] = {0x01,0x05};
...
void char1_read_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	if (gl_char.char_val!=NULL) {
		ESP_LOGI(GATTS_TAG, "char1_read_handler char_val %d\n",gl_char.char_val->attr_len);
		rsp.attr_value.len = gl_char.char_val->attr_len;
		for (uint32_t pos=0;pos<gl_char.char_val->attr_len&&pos<gl_char.char_val->attr_max_len;pos++) {
			rsp.attr_value.value[pos] = gl_char.char_val->attr_value[pos];
		}
	}
...
void descr1_read_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
	rsp.attr_value.handle = param->read.handle;
	if (gl_char.descr_val!=NULL) {
		ESP_LOGI(GATTS_TAG, "descr1_read_handler descr_val %d\n",gl_char.descr_val->attr_len);
		rsp.attr_value.len = gl_char.descr_val->attr_len;
		for (uint32_t pos=0;pos<gl_char.descr_val->attr_len&&pos<gl_char.descr_val->attr_max_len;pos++) {
			rsp.attr_value.value[pos] = gl_char.descr_val->attr_value[pos];
		}
	}
...
static void notify_handle_get_msg(esp_gatt_if_t gatts_if, uint16_t conn_id) {
	// inform.date_time = 1;
	// inform.latitude = 2.0f;
	// inform.longitude = 3.0f;
	// esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &rsp);
	// char msg[20] = "hello i'm dien";
	extern Inform inform;
	char msg[20];

	sprintf(msg, "%f,%f", inform.latitude, inform.longitude);
	// esp_ble_gatts_send_indicate(gatts_if, conn_id, gl_char.char_handle,sizeof(Inform),(uint8_t *)&inform,false);
	esp_ble_gatts_send_indicate(gatts_if, conn_id, gl_char.char_handle,strlen(msg),(uint8_t *)msg,false);
	...
	// 	notify_pos='0';
		...
		// 	ESP_LOGI(GATTS_TAG, "notify_handle esp_ble_gatts_send_indicate\n");
...
static void notify_handle_get_raw_msg(esp_gatt_if_t gatts_if, uint16_t conn_id) {
	extern Inform inform;
	esp_ble_gatts_send_indicate(gatts_if, conn_id, gl_char.char_handle,sizeof(Inform),(uint8_t *)&inform,false);
	ESP_LOGI(GATTS_TAG, "notify_handle_get_raw_msg\n");
}
...
void char1_write_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	if (gl_char.char_val!=NULL) {
		...
		gl_char.char_val->attr_len = param->write.len;
		for (uint32_t pos=0;pos<param->write.len;pos++) {
			gl_char.char_val->attr_value[pos]=param->write.value[pos];
		}
		ESP_LOGI(TAG, "char1_write_handler %.*s", gl_char.char_val->attr_len, (char*)gl_char.char_val->attr_value);
	}
    ...
    if (strncmp((const char *)gl_char.char_val->attr_value,"LED ON",6)==0) {
    	gpio_set_level(LED_PIN,LOW);
    	led_stat=1;
    } else if (strncmp((const char *)gl_char.char_val->attr_value,"LED OFF",7)==0) {
    	gpio_set_level(LED_PIN,HIGH);
    	led_stat=0;
    } else if (strncmp((const char *)gl_char.char_val->attr_value,"LED SWITCH",10)==0) {
    	led_stat=1-led_stat;
    	gpio_set_level(LED_PIN,led_stat);
    } else if (strncmp((const char *)gl_char.char_val->attr_value,"GET RAW",7)==0) {
		notify_handle_get_raw_msg(gatts_if, param->write.conn_id);
	} else if (strncmp((const char *)gl_char.char_val->attr_value,"GET",3)==0) {
		notify_handle_get_msg(gatts_if, param->write.conn_id);
	} else {
		ESP_LOGI(GATTS_TAG, "notify_handle esp_ble_gatts_send_indicate\n");
    }
}
...
void descr1_write_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	if (gl_char.descr_val!=NULL) {
		...
		gl_char.descr_val->attr_len = param->write.len;
		for (uint32_t pos=0;pos<param->write.len;pos++) {
			gl_char.descr_val->attr_value[pos]=param->write.value[pos];
		}
	}
...
void gatts_add_char() {
	...
	for (uint32_t pos=0;pos<GATTS_CHAR_NUM;pos++) {
		if (gl_char.char_handle==0) {
			ESP_LOGI(GATTS_TAG, "ADD pos %d handle %d service %d\n", pos,gl_char.char_handle,gl_profile.service_handle);
			ble_add_char_pos=pos;
			esp_ble_gatts_add_char(gl_profile.service_handle, &gl_char.char_uuid,
								   gl_char.char_perm,gl_char.char_property,gl_char.char_val, gl_char.char_control);
			break;
		}
	}
}
...
void gatts_check_add_char(esp_bt_uuid_t char_uuid, uint16_t attr_handle) {
	...
	if (attr_handle != 0) {
		...
		gl_char.char_handle=attr_handle;

		// is there a descriptor to add ?
		if (gl_char.descr_uuid.len!=0 && gl_char.descr_handle==0) {
			ESP_LOGI(GATTS_TAG, "ADD Descr pos %d handle %d service %d\n", ble_add_char_pos,gl_char.descr_handle,gl_profile.service_handle);
			esp_ble_gatts_add_char_descr(gl_profile.service_handle, &gl_char.descr_uuid,
					gl_char.descr_perm, gl_char.descr_val, gl_char.descr_control);
		} else {
			gatts_add_char();
		}
	}
...
void gatts_check_add_descr(esp_bt_uuid_t descr_uuid, uint16_t attr_handle) {
	...
	if (attr_handle != 0) {
		...
		gl_char.descr_handle=attr_handle;
	}
	gatts_add_char();
}
...
void gatts_check_callback(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
	...
	for (uint32_t pos=0;pos<GATTS_CHAR_NUM;pos++) {
		if (gl_char.char_handle==handle) {
			if (read==1) {
				if (gl_char.char_read_callback!=NULL) {
					gl_char.char_read_callback(event, gatts_if, param);
				}
			} else {
				if (gl_char.char_write_callback!=NULL) {
					gl_char.char_write_callback(event, gatts_if, param);
				}
			}
			break;
		}
		if (gl_char.descr_handle==handle) {
			if (read==1) {
				if (gl_char.descr_read_callback!=NULL) {
					gl_char.descr_read_callback(event, gatts_if, param);
				}
			} else {
				if (gl_char.descr_write_callback!=NULL) {
					gl_char.descr_write_callback(event, gatts_if, param);
				}
			}
			break;
		}
...
//       esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        gl_profile.char_uuid.len = gl_char.char_uuid.len;
        gl_profile.char_uuid.uuid.uuid16 = gl_char.char_uuid.uuid.uuid16;
